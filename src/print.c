...
/*   Updated: 2022/09/14 08:36:06 by mhaddaou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void printFlorCiel(t_cub *cub)
{
     int y = cub->ry / 2;
    int i = 0;
    int j;
    int clrC = createRGB(cub->map->C.R, cub->map->C.G, cub->map->C.B);
    while (i  < y)
    {
        j = 0;
        while (j < cub->rx)
        {
            my_mlx_pixel_put(&cub->data,j, i, 0x00D5FF );
            j++;
        }
        i++;
    }
